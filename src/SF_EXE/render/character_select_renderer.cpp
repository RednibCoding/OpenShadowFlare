...
void renderNameEditor(
    ...
    const std::int32_t caretColumn =
        std::min<std::int32_t>(textCellCount(data.character_name), 20);
    renderer.drawRectangle(
    ...
    for (std::size_t index = 0; index < 6; ++index) {
        ...
        const std::int32_t itemBrightness =
            index ==
                    static_cast<std::size_t>(
                        std::max<std::int32_t>(selected, 0))
                ? brightness
                : brightness / 2;
        const bool selectedItem =
            index ==
            static_cast<std::size_t>(std::max<std::int32_t>(selected, 0));
        ...
        if (hovered) {
            const std::int32_t renderedCounter =
                std::max<std::int32_t>(data.save_hover_animation - 1, 0);
            numberFrame =
